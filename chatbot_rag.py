...
class RAGSystem:
    def __init__(self):
        ...
        self.doc_hash_file = ".doc_hash"  # Archivo para guardar hash de documentos
        
    def get_documents_hash(self, folder_path="./documentos"):
        """Genera un hash de los documentos actuales (nombre + tamaño + fecha)"""
        try:
    ...
    def documents_changed(self, folder_path="./documentos"):
        """Verifica si los documentos han cambiado desde la última procesamiento"""
        try:
            ...
            return True  # Si hay error, asumir que cambió
    ...
    def generate_response(self, query, context_docs):
        ...
        # 1. DEFINIMOS LAS REGLAS (SYSTEM PROMPT)
        # Aquí van las instrucciones de comportamiento y límites.
...
"""
        ...
        # 2. PREPARAMOS EL INPUT (USER PROMPT)
        # Aquí va la "materia prima" y la duda del usuario.
        user_content = f"""
[CONTEXTO]
{context}
...
"""
        ...
        response = ollama.chat(
            ...
            # IMPORTANTE: Temperatura baja para evitar alucinaciones
            options={
                'temperature': 0.1, 
                'top_k': 10
            },
...
def get_mensaje_motivacional():
    """Retorna mensajes motivacionales aleatorios"""
    mensajes = [
...
def get_frase_del_dia():
    """Retorna una frase motivacional del día"""
    frases = [
...
def generar_preguntas_quiz():
    """Genera preguntas de quiz basadas en conceptos clave"""
    preguntas_quiz = [
        {
        ...
        },
        {
            ...
            "respuesta_correcta": 1,
            "explicacion": "La UE busca la unión política y económica de sus estados miembros para la paz y prosperidad."
        },
        {
        ...
        },
        {
            ...
            "respuesta_correcta": 2,
            "explicacion": "El Tratado de Maastricht de 1992 transformó la CEE en la Unión Europea."
        },
        {
            ...
            "respuesta_correcta": 1,
            "explicacion": "La UE tiene mayor institucionalización que TLCAN. Mercosur está entre ambos niveles."
        }
    ]
...
def main():
    ...
    with st.sidebar:
        ...
        # Botón para procesar documentos (solo mostrar si cambió algo)
    ...
    if st.session_state.quiz_activo:
        ...
        return  # Terminar aquí si quiz está activo
    ...
    if st.session_state.generate_response_flag and st.session_state.messages:
        ...
        if not st.session_state.docs_processed:
            st.warning("⚠️ Por favor, procesa los documentos primero usando el botón en el menú lateral")
        else:
            with st.chat_message("assistant"):
                with st.spinner("Buscando información..."):
                    try:
                        # Buscar contexto relevante (5 fragmentos para mejor cobertura)
                        ...
                        if results['documents'] and results['documents'][0]:
                            ...
                            for chunk in st.session_state.rag.generate_response(prompt, context_docs):
                            ...
                            st.error("No se encontró información relevante. Intenta reformular tu pregunta.")
                    except Exception as e:
    ...
    if prompt := st.chat_input("Escribe tu pregunta aquí..."):
        ...
        with st.chat_message("assistant"):
            with st.spinner("Buscando información..."):
                try:
                    # Buscar contexto relevante (10 fragmentos para mejor cobertura)
                    results = st.session_state.rag.search(prompt, n_results=10)
